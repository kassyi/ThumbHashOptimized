...
export class BenchUI {
  ...
  constructor() {
    ...
    this.profilerBars = document.getElementById("profilerBars") as HTMLElement;
    this.totalProfileTimeEl = document.getElementById("totalProfileTime") as HTMLElement;
    this.jsonOutput = document.getElementById("jsonOutput") as HTMLTextAreaElement;
    this.copyJsonBtn = document.getElementById("copyJsonBtn") as HTMLButtonElement;
    this.copyFeedback = document.getElementById("copyFeedback") as HTMLElement;
    this.verifyBadge = document.getElementById("verifyBadge") as HTMLElement;

    this.origEncodeTimeEl = document.getElementById("origEncodeTime") as HTMLElement;
    this.optEncodeTimeEl = document.getElementById("optEncodeTime") as HTMLElement;
    this.encodeSpeedupBadge = document.getElementById("encodeSpeedupBadge") as HTMLElement;

    this.origDecodeTimeEl = document.getElementById("origDecodeTime") as HTMLElement;
    this.optDecodeTimeEl = document.getElementById("optDecodeTime") as HTMLElement;
    this.decodeSpeedupBadge = document.getElementById("decodeSpeedupBadge") as HTMLElement;
  ...
  private initEventListeners() {
    ...
    this.copyJsonBtn.addEventListener("click", () => this.copyToClipboard(this.jsonOutput.value));
  ...
  private updateProfilerUI() {
    ...
    for (const [key, stage] of Object.entries(profiler.stages)) {
      ...
      setTimeout(() => {
        (item.querySelector(".progress-bar") as HTMLElement).style.width = `${percentage}%`;
      }, 50);
    }
  }
  ...
  private async runBenchmark() {
    ...
    // Clear times
    [this.origEncodeTimeEl, this.optEncodeTimeEl, this.origDecodeTimeEl, this.optDecodeTimeEl].forEach(el => el.textContent = "...");
    [this.encodeSpeedupBadge, this.decodeSpeedupBadge].forEach(el => el.classList.add("hidden"));
    ...
    if (isExactMatch) {
      this.verifyBadge.textContent = "Data Integrity: Passed (Bit-Exact Match) ✅";
      this.verifyBadge.className = "text-[10px] font-bold text-emerald-900 bg-emerald-400 px-4 py-1 rounded-full shadow-lg";
    } else {
      this.verifyBadge.textContent = "Data Integrity: Passed (Visually Identical, Float Delta Detected) ⚠️";
      this.verifyBadge.className = "text-[10px] font-bold text-yellow-900 bg-yellow-400 px-4 py-1 rounded-full shadow-lg";
    }
    ...
    for (let i = 0; i < ITERATIONS; i++) this.originalStrategy.rgbaToThumbHash(w, h, dummyData);
    ...
    for (let i = 0; i < ITERATIONS; i++) this.originalStrategy.thumbHashToRGBA(hashOrig);
    ...
    for (let i = 0; i < ITERATIONS; i++) this.optimizedStrategy.rgbaToThumbHash(w, h, dummyData);
    ...
    for (let i = 0; i < ITERATIONS; i++) this.optimizedStrategy.thumbHashToRGBA(hashOpt);
    ...
    const exportData = {
      ...
      profiles: Object.fromEntries(
        Object.entries(this.currentProfilerData).map(([k, p]) => [k, p.times])
      )
    };
  ...
  private copyToClipboard(text: string, isAuto = false) {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard.writeText(text).then(executeCopy).catch(() => this.fallbackCopy(text, executeCopy));
    } else {

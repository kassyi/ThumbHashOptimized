...
export class ThumbHashOptimized implements IThumbHashStrategy {
    ...
    constructor() {
        if (__PROFILE__) {
            this.encodeProfiler = new Profiler({
                setupAvg: {
                    label: "1. Fast Opaque Scan & Avg",
                    color: "bg-indigo-500",
                },
                tables: {
                    label: "2. Math.cos LUT Generation",
                    color: "bg-blue-500",
                },
                encodeHorizL: {
                    label: "3. Fission DCT (Horiz L)",
                    color: "bg-purple-500",
                },
                encodeHorizPQ: {
                    label: "4. Fission DCT (Horiz PQ)",
                    color: "bg-fuchsia-500",
                },
                encodeHorizA: {
                    label: "5. Fission DCT (Horiz A)",
                    color: "bg-pink-500",
                },
                encodeVertL: {
                    label: "6. Vert 1D DCT (L)",
                    color: "bg-emerald-500",
                },
                encodeVertPQ: {
                    label: "7. Vert 1D DCT (Color)",
                    color: "bg-teal-500",
                },
                encodeVertA: {
                    label: "8. Vert 1D DCT (Alpha)",
                    color: "bg-cyan-500",
                },
                packing: {
                    label: "9. Bit Packing & Shift",
                    color: "bg-gray-500",
                },
            });
    ...
    rgbaToThumbHash(
        ...
        let hasAlpha = false;
        for (
            let i = 3, len = numPixels * RGBA_CHANNELS;
            i < len;
            i += RGBA_CHANNELS
        )
            if (rgba[i] < 255) {
        ...
        const rgba32 = isLittleEndian
            ? new Uint32Array(rgba.buffer, rgba.byteOffset, numPixels)
            : null;
        ...
        const nx_l = max(
                1,
                round(
                    ((hasAlpha ? MAX_ALPHA_FREQ : MAX_L_FREQ) * w) / max(w, h),
                ),
            ),
            ny_l = max(
                1,
                round(
                    ((hasAlpha ? MAX_ALPHA_FREQ : MAX_L_FREQ) * h) / max(w, h),
                ),
            );
        ...
        if (
            ...
            for (let cx = 0; cx < mnx; cx++) {
                ...
                for (let x = 0; x < endX; x++)
                    fxTable[x * MAX_L_FREQ + cx] = cos(f * (x + 0.5));
            }
        ...
        if (!hasAlpha && isLittleEndian && rgba32) {
            for (let y = 0; y < h; y++) {
                ...
                for (
                    let x = 0;
                    x < halfWEven;
                    x += 2, srcL += 2, srcR -= 2, fxIdx += 2 * MAX_L_FREQ
                ) {
                    ...
                    const sL1 =
                        ...
                        sR1 =
                            ...
                            ((pR1 >> 16) & 0xff);
                    sl0 +=
                        (sL0 + sR0) * fxTable[fxIdx] +
                        (sL1 + sR1) * fxTable[fxIdx + MAX_L_FREQ];
                    sl1 +=
                        (sL0 - sR0) * fxTable[fxIdx + 1] +
                        (sL1 - sR1) * fxTable[fxIdx + MAX_L_FREQ + 1];
                    sl2 +=
                        (sL0 + sR0) * fxTable[fxIdx + 2] +
                        (sL1 + sR1) * fxTable[fxIdx + MAX_L_FREQ + 2];
                    sl3 +=
                        (sL0 - sR0) * fxTable[fxIdx + 3] +
                        (sL1 - sR1) * fxTable[fxIdx + MAX_L_FREQ + 3];
                    sl4 +=
                        (sL0 + sR0) * fxTable[fxIdx + 4] +
                        (sL1 + sR1) * fxTable[fxIdx + MAX_L_FREQ + 4];
                    sl5 +=
                        (sL0 - sR0) * fxTable[fxIdx + 5] +
                        (sL1 - sR1) * fxTable[fxIdx + MAX_L_FREQ + 5];
                    sl6 +=
                        (sL0 + sR0) * fxTable[fxIdx + 6] +
                        (sL1 + sR1) * fxTable[fxIdx + MAX_L_FREQ + 6];
                }
                if (halfWEven < halfW) {
            ...
            for (let y = 0; y < h; y++) {
                ...
                for (
                    ...
                    const PL1 = RL1 + GL1 - (BL1 << 1),
                        ...
                        QR1 = RR1 - GR1;
                    sp0 +=
                        (PL0 + PR0) * fxTable[fxIdx] +
                        (PL1 + PR1) * fxTable[fxIdx + MAX_L_FREQ];
                    sp1 +=
                        (PL0 - PR0) * fxTable[fxIdx + 1] +
                        (PL1 - PR1) * fxTable[fxIdx + MAX_L_FREQ + 1];
                    sp2 +=
                        (PL0 + PR0) * fxTable[fxIdx + 2] +
                        (PL1 + PR1) * fxTable[fxIdx + MAX_L_FREQ + 2];
                    sq0 +=
                        (QL0 + QR0) * fxTable[fxIdx] +
                        (QL1 + QR1) * fxTable[fxIdx + MAX_L_FREQ + 7];
                    sq1 +=
                        (QL0 - QR0) * fxTable[fxIdx + 1] +
                        (QL1 - QR1) * fxTable[fxIdx + MAX_L_FREQ + 8];
                    sq2 +=
                        (QL0 + QR0) * fxTable[fxIdx + 2] +
                        (QL1 + QR1) * fxTable[fxIdx + MAX_L_FREQ + 9];
                }
                if (halfWEven < halfW) {
                    const pL = rgba32[srcL],
                        pR = rgba32[srcR],
        ...
        for (let cy = 0; cy < ny_l; cy++) {
            ...
            for (let cx = 0; cx * ny_l < nx_l * (ny_l - cy); cx++) {
                let f = 0,
                    rso = cx * h;
                for (let y = 0; y < h; y++)
                    f += rowSumsL[rso + y] * fyTable[fyo + y];
                f /= numPixels;
                if (cx || cy) {
                    l_ac.push(f);
        ...
        for (let cy = 0; cy < MAX_CHROMA_FREQ; cy++) {
            ...
            for (
                let cx = 0;
                cx * MAX_CHROMA_FREQ < MAX_CHROMA_FREQ * (MAX_CHROMA_FREQ - cy);
                cx++
            ) {
                let fp = 0,
                    fq = 0,
        ...
        if (hasAlpha)
            hash.push(
                round((ALPHA_MAX * avg_a) / numPixels) |
                    (round(ALPHA_MAX * 0) << 4),
            );
        for (const ac of [l_ac, p_ac, q_ac])
            for (const f of ac)
                hash[ac_start + (ac_idx >> 1)] |=
                    round(ALPHA_MAX * f) << ((ac_idx++ & 1) << 2);
    ...
    thumbHashToRGBA(hash: Uint8Array): ThumbHashImage {
        ...
        const LB = (h24 & (L_DC_MAX | 0)) * DEC_L_DC_PRESCALE,
            PB =
                (((h24 >> H24_P_DC) & (L_DC_MAX | 0)) * DEC_PQ_DC_NORM - 1.0) *
                DEC_PQ_BASE_VAL,
            QB =
                (((h24 >> H24_Q_DC) & (L_DC_MAX | 0)) * DEC_PQ_DC_NORM - 1.0) *
                DEC_Q_BASE_VAL;
        const lS =
                ((h24 >> H24_L_SCALE) & (L_SCALE_MAX | 0)) * DEC_L_AC_PRESCALE,
            hasA = (h24 >> H24_HAS_ALPHA) & 1,
            pS =
                ((h16 >> H16_P_SCALE) & (PQ_SCALE_MAX | 0)) * DEC_P_AC_PRESCALE,
            qS =
                ((h16 >> H16_Q_SCALE) & (PQ_SCALE_MAX | 0)) * DEC_Q_AC_PRESCALE;
        ...
        const w = round(
                lx / ly > 1 ? MAX_DECODE_DIM : MAX_DECODE_DIM * (lx / ly),
            ),
            h = round(
                lx / ly > 1 ? MAX_DECODE_DIM / (lx / ly) : MAX_DECODE_DIM,
            );
        ...
        if (w !== lastDW || h !== lastDH || lx !== lastDLX || ly !== lastDLY) {
            for (let cx = 0; cx < max(lx, MAX_CHROMA_FREQ + 2); cx++) {
                const f = (PI / w) * cx;
                for (let x = 0; x < w; x++)
                    dFx[cx * w + x] = cos(f * (x + 0.5));
            }
            for (let cy = 0; cy < max(ly, MAX_CHROMA_FREQ + 2); cy++) {
                const f = (PI / h) * cy;
                for (let y = 0; y < h; y++)
                    dFy[cy * h + y] = cos(f * (y + 0.5)) * 2.0;
            }
            lastDW = w;
            lastDH = h;
            lastDLX = lx;
        ...
        if (ly === MAX_CHROMA_FREQ) {
            for (let y = 0; y < halfH; y++) {
                ...
                for (
                    let x = 0;
                    x < w;
                    x++, it += RGBA_CHANNELS, ib += RGBA_CHANNELS
                ) {
            ...
            for (let y = 0; y < halfH; y++) {
                ...
                for (
                    ...
                    const lE =
                            LB + tL[x] * f0 + tL[w2 + x] * f2 + tL[w4 + x] * f4,
            ...
            for (let y = 0; y < halfH; y++) {
                ...
                for (
                    ...
                    const lE =
                            ...
                            tL[w6 + x] * f6,
                        lO =
                            tL[w1 + x] * f1 + tL[w3 + x] * f3 + tL[w5 + x] * f5,
                        pE = PB + tP[x] * f0 + tP[w2 + x] * f2,
                        pO = tP[w1 + x] * f1,
                        qE = QB + tQ[x] * f0 + tQ[w2 + x] * f2,
                        qO = tQ[w1 + x] * f1;

    ...
    constructor() {
        if (__PROFILE__) {
            this.encodeProfiler = new Profiler({
                process: {
                    label: "Legacy Encode Process",
                    color: "bg-gray-500",
                },
            });
            this.decodeProfiler = new Profiler({
                process: {
                    label: "Legacy Decode Process",
                    color: "bg-gray-400",
                },
            });
        }
    ...
    thumbHashToRGBA(hash: Uint8Array): ThumbHashImage {
        ...
        for (let y = 0, i = 0; y < h; y++) {
            for (let x = 0; x < w; x++, i += 4) {
                ...
                for (let cx = 0; cx < max(lx, hasAlpha ? 5 : 3); cx++)
                    fx[cx] = cos((PI / w) * (x + 0.5) * cx);
                for (let cy = 0; cy < max(ly, hasAlpha ? 5 : 3); cy++)

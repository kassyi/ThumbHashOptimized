export class DecodeVertGenerator {
  static generateIDCTVert(maxLFreq: number, maxChromaFreq: number): string {
    ...
    ${branch} {
      ...
      let fVars = [];
      for (let i = 0; i < _ly; i++) fVars.push(`const f${i} = dFy[${i ? i + " * " : ""}h + y];`);
      ...
      const chDefs = channels.map(c => {
        let e = c.base + ' + ' + c.src + '[x] * f0';
        for (let i = 2; i < c.freq; i += 2) e += ' + ' + c.src + '[w' + i + ' + x] * f' + i;
        let o = c.src + '[w1 + x] * f1';
        for (let i = 3; i < c.freq; i += 2) o += ' + ' + c.src + '[w' + i + ' + x] * f' + i;
        return `${c.id}E = ${e},\n            ${c.id}O = ${o}`;
      }).join(',\n            ');
  ...
  static generateIDCTVertOdd(maxLFreq: number, maxChromaFreq: number): string {
    let out = '';
    for (let _ly = 3; _ly <= maxLFreq; _ly++) {
      let branch = _ly === 3 ? "if (ly === 3)" : "else " + (_ly === maxLFreq ? "" : `if (ly === ${_ly})`);
      let fVars = [`f0 = dFy[y]`];
      for (let i = 2; i < _ly; i += 2) fVars.push(`f${i} = dFy[${i} * h + y]`);
